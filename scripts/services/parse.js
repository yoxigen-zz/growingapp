define(["angular", "parse", "services/phonegap"], function(angular, Parse){
    "use strict";

    angular.module("Parse", ["Phonegap"]).factory("parse", parseService);

    parseService.$inject = ["$q", "$rootScope", "phonegap"];

    /**
     * Service for working with the Parse SDK
     * @returns {{facebookLogin: facebookLogin, get: get, getCurrentUser: getCurrentUser, init: init, login: login, logout: logout, query: query, remove: remove, removeAll: removeAll, runFunction: runFunction, save: save, saveAll: saveAll, signUp: signUp, uploadBase64ToFile: uploadBase64ToFile, uploadFile: uploadFile}}
     */
    function parseService($q, $rootScope, phonegap){
        var parseConfig = {
